...
It took a lot of experimenting to get this to work particularly for the continue case since it requires pushing a new behavior tree to the stack and there are rules to when you can push a new behavior tree instance to the stack which requires some task level observation to get the timing right.
