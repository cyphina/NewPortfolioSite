...
* If we don't think the circumstances have changed then just **continue** with the behavior tree where we left off before the reactive behavior tree.
  * Requires careful management of the events that lead to the reactive subtree cause you have to stop the event before you push the reactive subtree and start it again afterwards.
...
There's also how you handle reacts aka when you start the reactive behavior tree. In my system you can:

* Do it instantly. So if you're casting a spell you just stop.
* Queue it after a task ends.
