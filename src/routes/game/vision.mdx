...
>I think technically reading the positions we use for the line trace is not thread-safe. The transform data can be modified on the main thread (like the transform being written to every frame during movement) and reading it off-thread could in rare cases result in corrupted or inconsistent values from tearing. That said I haven't' seen any problems in practice even when we have a lot of units moving around, and teleporting.
>
...
The vision thread has a message queue. The main thread uses it to communicate to the vision thread about when units die so we can clean up the bookkeeping for groups and individual units.
