...
If you talk to an NPC with either of these components after the initial dialog a menu opens up with more options based on what components were added and the camera changes to zoom in on the NPC.
...
2. For the shop component - list of items to sell and conditions on when they are unlocked. Since we didn't use observers here if the conditions are somehow met you have to close and reopen the shopa which prompts the system to reevaluate them.
...
>The data assets are used since we might create several versions of the NPCs and swap out their look but we can reuse the data asset to reuse the item lists or use inheritance on them to expand upon what an NPC is selling as the game progresses.
